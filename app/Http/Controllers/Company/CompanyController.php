...
class CompanyController extends Controller
{
	...
	public function makeDeal(Request $request, $application_id, $user_id, $job_id, $company_id)
    {
		$data['user_id'] = $user_id;
		$data['job_id'] = $job_id;
		$data['company_id'] = $company_id;

        //$JobApply = JobApply::find($application_id);


        $user = User::find($user_id);
        $job = Job::find($job_id);
        if($job->num_of_positions != 0){
         $job->num_of_positions = ($job->num_of_positions - 1);
        }else{
         $job->job_status_id = 3;
        }
        ...
        if($job->num_of_positions == 0){
            	flash(__('max num_of_positions'))->error();
                return back();
            }
        ...
        $Employee->save();
          	...
          	$data['contract_status_id'] = 5;

        $data_save = Contract::create($data);
	...
	public function postedJobs(Request $request)
    {
          		...
          		$jobs = Auth::guard('company')->user()->jobs()->

                Where(function ($query) use ($request) {
   if (isset($request->job_status_id)) {
                $query->where('jobs.job_status_id', $request->job_status_id );
                 }} )->
                paginate(10);
