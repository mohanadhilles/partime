...
class Job extends Model
{
    ...
    {
        return $this->belongsTo('App\JobStatus', 'job_status_id', 'job_status_id');
    }

    public function getJobStatus($field = '')
    {
		$JobStatus = $this->JobStatus()->lang()->first();
		if(null === $JobStatus){
			$JobStatus = $this->JobStatus()->first();
		}
        if(null !== $JobStatus){
        	if (!empty($field)) {
				return $JobStatus->$field;
			} else {
				return $JobStatus;
			}
		}
	...
	/********************************/
    	...
    	/*********************************/
	public function FavouriteAppliedUsers()
    {
        return $this->hasMany('App\FavouriteApplicant', 'job_id', 'id');
    }

    public function getFavouriteAppliedUserIdsArray()
    {
        return $this->FavouriteAppliedUsers->pluck('user_id')->toArray();
    }
	/********************************/
