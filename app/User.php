...
class User extends Authenticatable
{
    ...
    {
		$careerLevel = $this->Country()->lang()->first();
		if(null === $careerLevel){
			$careerLevel = $this->Country()->first();
		}
        if(null !== $careerLevel){
            if (!empty($field))
                return $careerLevel->$field;
    ...
    {
        return $this->belongsTo('App\Country', 'nationality_id', 'country_id');
    }

    public function getNationality($field = '')
    {
		$careerLevel = $this->Nationality()->lang()->first();
		if(null === $careerLevel){
			$careerLevel = $this->Nationality()->first();
		}
        if(null !== $careerLevel){
            if (!empty($field))
                return $careerLevel->$field;
