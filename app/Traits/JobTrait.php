...
trait JobTrait
{
	...
	private function assignJobValues($job, $request)
	{
      ...
      //  $job->is_freelance = $request->input('is_freelance');
       // $job->career_level_id = $request->input('career_level_id');
        ...
        $job->contract_period_id = $request->input('contract_period_id');
        $job->working_hours  = (int)$request->input('working_hours');
        $job->work_days  = (int)$request->input('work_days');
        $job->expected_date  =  $request->input('expected_date');
        $job->candidate_working  =  $request->input('candidate_working');
        $job->candidate_was_working  =  $request->input('candidate_was_working');
    ...
    public function editJob($id)
    {
		...
		$salaryPeriods = DataArrayHelper::defaultSalaryPeriodsArray();
        		...
        		$contractPeriods = DataArrayHelper::langContractPeriodsArray();
...
*/
    ...
    public function editFrontJob($id)
    {
     ...
     $job = Job::findOrFail($id);
        $countries = DataArrayHelper::langCountriesArray();
        $cities = DataArrayHelper::AllCitiesArray($job->country_id);
        $currencies = DataArrayHelper::currenciesArray();
		$careerLevels = DataArrayHelper::langCareerLevelsArray();
		$functionalAreas = DataArrayHelper::langFunctionalAreasArray();
		$jobTypes = DataArrayHelper::langJobTypesArray();
